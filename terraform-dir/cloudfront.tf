...
resource "aws_cloudfront_distribution" "cdn_static_website" {
  ...
  logging_config {
    ...
    bucket          = aws_s3_bucket.s3-bucket.bucket_regional_domain_name
...
output "cloudfront_url" {
  value ="Your cloudfront url is  ${aws_cloudfront_distribution.cdn_static_website.domain_name}"
}

# Output a message with the website address
output "website_message" {
  value = "Your website is ready. Click here: https://${var.domain-name}"
}
